...
class HeadlessBrowser():
    ...
    def __init__(self, useragent=None):
        ...
        self.driver.execute_cdp_cmd('Network.setUserAgentOverride', {'userAgent':
                                                                         useragent or self.driver.execute_script(
    ...
    def generate(self, fuzzers=[]):
        ...
        for f_name in fuzzers or [
            ...
            try:
                ...
                for domain in f():
                    self.domains.add(
                        Permutation(fuzzer=f_name, domain='.'.join(filter(None, [self.subdomain, domain, self.tld]))))
        if not fuzzers or 'tld-swap' in fuzzers:
            for tld in self._tld():
                self.domains.add(
                    Permutation(fuzzer='tld-swap', domain='.'.join(filter(None, [self.subdomain, self.domain, tld]))))
...
class Format():
    ...
    def cli(self):
        ...
        for domain in domains:
            inf = []
            if 'dns_a' in domain:
                inf.append(';'.join(domain['dns_a']) + (
                    kv('/', domain['geoip'].replace(' ', '')) if 'geoip' in domain else ''))
            ...
            cli.append('{}{[fuzzer]:<{}}{} {[domain]:<{}} {}'.format(FG_BLU, domain, wfuz, FG_RST, domain, wdom,
                                                                     ' '.join(inf or ['-'])))
        return '\n'.join(cli)
...
def run(**kwargs):
    ...
    parser.add_argument('-f', '--format', type=str, default='cli',
                        help='Output format: cli, csv, json, list (default: cli)')
    parser.add_argument('--fuzzers', type=str, metavar='LIST',
    ...
    if args.ssdeep:
        p_err('WARNING: argument --lsh ssdeep is deprecated, use --lsh ssdeep instead')
        args.lsh = 'ssdeep'
    if args.ssdeep_url:
        p_err('WARNING: argument --lsh ssdeep-url is deprecated, use --lsh-url instead')
    ...
    if args.tld:
        try:
            with open(args.tld, encoding='utf-8') as f:
                tld = [x for x in set(f.read().splitlines()) if
                       re.match(r'^[a-z0-9-]{2,63}(\.[a-z0-9-]{2,63}){0,1}$', x)]
        except FileNotFoundError:
    ...
    while True:
        ...
        p_cli(ST_CLR + '\rpermutations: {:.2%} of {} | found: {} | eta: {:d}m {:02d}s | speed: {:d} qps'.format(
            comp / dlen,
